...
export async function generateMetadata() {
  ...
  const headerLocale = accepted.split(",")[0].split("-")[0];
  const locale = (cookieLocale || headerLocale || "en").startsWith("pt") ? "pt" : "en";
...
export default async function RootLayout({
  ...
  const cookieLocale = cookieStore.get("locale")?.value;
  const headerLocale = accepted.split(",")[0].split("-")[0];
  const locale = (cookieLocale || headerLocale || "en").startsWith("pt")
    ? "pt"
    : "en";
  const messages = await loadMessages(locale);
  return (
